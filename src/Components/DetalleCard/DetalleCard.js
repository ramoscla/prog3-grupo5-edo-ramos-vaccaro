...
class DetalleCard extends Component {
  ...
  componentDidMount() {
    fetch(
      `https://api.themoviedb.org/3/movie/${this.props.id}?api_key=31e421d77201e7a1eefe33f85b67fa3b`)
  ...
  quitarDeFavoritos() {
    const storage = localStorage.getItem("favoritos");
    const parsedStorage = JSON.parse(storage);
    const restoFavoritos = parsedStorage.filter(
      (peli) => peli !== parseInt(this.props.id)
    );
      ...
      <>
        <div className="Detalle">
          <img
            ...
            src={`https://media.themoviedb.org/t/p/w600_and_h900_bestv2/${this.state.pelicula.backdrop_path}`}
            alt="title-poster"
          ></img>
